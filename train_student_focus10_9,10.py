...
def parse_option():
    ...
    # 蒸馏参数
    parser.add_argument('--distill', type=str, default='kd', choices=['kd', 'crd'], help='蒸馏方法')
    ...
    parser.add_argument('--min_temp', type=float, default=4, help='动态温度最小值初始值')
    parser.add_argument('--max_temp', type=float, default=4, help='动态温度最大值初始值')
    ...
    parser.add_argument('--min_temp_min', type=float, default=4, help='min_temp 的最小允许值')
    parser.add_argument('--min_temp_max', type=float, default=4, help='min_temp 的最大允许值')
    parser.add_argument('--max_temp_min', type=float, default=4, help='max_temp 的最小允许值')
    parser.add_argument('--max_temp_max', type=float, default=4, help='max_temp 的最大允许值')

    # 实验追踪
    parser.add_argument('--trial', type=str, default='固定温度4', help='实验参数')
...
def main():
    ...
    # 循环训练模型6到10
    for model_index in range(6, 10):  # model_index 从 5 到 9，对应模型 6 到 10
        target_classes = target_class_groups[model_index]
        ...
        if opt.distill == 'crd' and opt.beta > 0:
        ...
        model_name_parts.append(temp_info)
        model_name_parts.append(f'trial_{opt.trial}')
        model_name_parts.append(f'model_{model_index+1}')
        ...
        if opt.dataset == 'cifar100':
            if opt.distill == 'crd':
                ...
                train_loader, val_loader, n_data = get_cifar100_dataloaders_sample(
                    batch_size=opt.batch_size, num_workers=opt.num_workers, k=16384, mode='exact'
                )
            else:
        ...
        criterion_kd = None
        if opt.distill == 'crd' and opt.beta > 0 and CRDLoss is not None:
