...
def main():
    ...
    for model_index, target_classes in enumerate(target_class_groups):
        ...
        model_name_parts = [
            ...
            opt.dataset,
            opt.distill,
            f'SC{model_index}',  # 修改为SC{model_index}格式
            f'r_{opt.gamma}',
            f'a_{opt.alpha}',
        ]
        if opt.beta > 0:
        ...
        # 创建加权损失函数 - 修改此处
        class_weights = torch.zeros(n_cls, device=device)  # 所有类别权重默认为0
        for cls in target_classes:
            class_weights[cls] = 1.0  # 目标类别权重设为1
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            if test_acc >= best_acc:
                ...
                save_file = os.path.join(opt.save_folder,
                                         f'{opt.model_s}_best_SC{model_index}.pth')
                try:
            ...
            if epoch % opt.save_freq == 0:
                state = {'epoch': epoch, 'model': model_s.state_dict(), 'accuracy': test_acc,
                         'optimizer': optimizer.state_dict()}
                save_file = os.path.join(opt.save_folder,
                                         f'ckpt_epoch_{epoch}_SC{model_index}.pth')
                try:
                    torch.save(state, save_file)
