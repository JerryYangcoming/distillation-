...
# 假设已经定义了 resnet8 和 resnet110
# 1. 加载预训练的 ResNet110 模型
teacher = resnet110(num_classes=100)
teacher_ckpt = torch.load(r'C:\Users\21985\Downloads\RepDistiller-master\RepDistiller-master\save\models\resnet110_vanilla\ckpt_epoch_240.pth', map_location='cpu')
teacher.load_state_dict(teacher_ckpt['model'])

# （2）准备student
student = resnet8(num_classes=100)

# （3）将teacher中的参数取出来，用于给student做初始化
teacher_dict = teacher.state_dict()
student_dict = student.state_dict()

transferred_count = 0
for name, param in teacher_dict.items():
    # 如果学生模型和教师模型中有相同名字 & 形状也一样，就复制
