...
def parse_option():
    ...
    parser.add_argument('--max_temp_min', type=float, default=10.0, help='max_temp 的最小允许值')
    ...
    # Grad-CAM 参数
    parser.add_argument('--gradcam_weight', type=float, default=0.1, help='Grad-CAM 差异损失权重')
    parser.add_argument('--gradcam_layer', type=int, default=3, help='用于 Grad-CAM 的层索引 (0-4)')
    ...
    trial_parts = [
        str(opt.min_temp), str(opt.max_temp), str(opt.temp_loss_threshold), str(opt.temp_adjust_factor),
        str(opt.min_temp_min), str(opt.min_temp_max), str(opt.max_temp_min), str(opt.max_temp_max)
    ]
...
def main():
    ...
    for model_index, target_classes in enumerate(target_class_groups):
        ...
        model_name_parts = [
            f'S_{opt.model_s}', f'T_{opt.model_t}', opt.dataset, opt.distill, f'SC{model_index}',
            f'r_{opt.gamma}', f'a_{opt.alpha}',
        ]
        if opt.beta > 0:
        ...
        criterion_kd = None
        criterion_gradcam = nn.MSELoss().to(device)  # 用于 Grad-CAM 差异损失
        ...
        criterion_list = nn.ModuleList([criterion_cls, criterion_div, criterion_kd, criterion_gradcam]).to(device)
        ...
        teacher_acc, _, _ = validate(val_loader, model_t, criterion_cls, opt, device=device,
        ...
        for epoch in range(1, opt.epochs + 1):
            ...
            train_acc, train_loss = train(epoch, train_loader, module_list, criterion_list, optimizer, opt, logger,
                                          min_temp, max_temp, target_classes=target_classes)
            test_acc, test_acc_top5, test_loss = validate(val_loader, model_s, criterion_cls, opt, device=device,
                                                          target_classes=target_classes)
        ...
        state = {'epoch': opt.epochs, 'model': model_s.state_dict(), 'accuracy': test_acc,
