...
def parse_option():
    ...
    parser.add_argument('--num_workers', type=int, default=32, help='数据加载线程数')
    ...
    parser.add_argument('--model_s', type=str, default='resnet20',
                        ...
                        help='学生模型架构')
    parser.add_argument('--path_t', type=str, default='save/models/resnet56_vanilla/ckpt_epoch_240.pth', help='教师模型预训练权重的路径')
    ...
    # 温度调整参数
    parser.add_argument('--temp_adjust_factor', type=float, default=0.9, help='温度范围调整因子')
    parser.add_argument('--min_temp_min', type=float, default=1, help='min_temp 的最小允许值')
    parser.add_argument('--min_temp_max', type=float, default=5, help='min_temp 的最大允许值')
    parser.add_argument('--max_temp_min', type=float, default=15, help='max_temp 的最小允许值')
    parser.add_argument('--max_temp_max', type=float, default=20, help='max_temp 的最大允许值')
...
def main():
    ...
    if opt.distill == 'kd':
        ...
        criterion_kd = nn.ModuleList([VIDLoss(s, t, t).to(device) for s, t in zip(s_n, t_n)])
    ...
    logger.log_value('teacher_acc', teacher_acc, 0)
